...
JsonHandler& JsonHandler::set_multi_json_lists(std::string key, std::vector<JsonHandler> multi) {
    std::vector<std::string> json_list;

    for (auto iter = multi.begin(); iter != multi.end(); iter++) {
        json_list.push_back(iter->to_str());
    }

    // call
    set_multi_json_lists(key, json_list);
